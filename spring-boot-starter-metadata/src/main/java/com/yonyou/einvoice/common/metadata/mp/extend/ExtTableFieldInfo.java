...
@ToString
@EqualsAndHashCode
public class ExtTableFieldInfo extends TableFieldInfo {
   ...
   * @deprecated v_3.1.2 please use {@link #insertStrategy} and {@link #updateStrategy} and {@link
   * #whereStrategy}
   */
  @Deprecated
  private final FieldStrategy fieldStrategy;
  /**
   * 字段验证策略之 insert Refer to {@link TableField#insertStrategy()}
   *
   * @since added v_3.1.2 @2019-5-7
   */
  private final FieldStrategy insertStrategy;
  /**
   * 字段验证策略之 update Refer to {@link TableField#updateStrategy()}
   *
   * @since added v_3.1.2 @2019-5-7
   */
  private final FieldStrategy updateStrategy;
  /**
   * 字段验证策略之 where Refer to {@link TableField#whereStrategy()}
   *
   * @since added v_3.1.2 @2019-5-7
   */
   ...
   */
  public ExtTableFieldInfo(GlobalConfig.DbConfig dbConfig, ExtTableInfo tableInfo,
      Field field,
      String column, String el, TableField tableField) {
    super(dbConfig, tableInfo, field, column, el, tableField);
    ...
    this.column = column;
    this.related = TableInfoHelper
        .checkRelated(tableInfo.isUnderCamel(), this.property, this.column);
   ...
   */
  public ExtTableFieldInfo(GlobalConfig.DbConfig dbConfig, ExtTableInfo tableInfo,
      Field field, TableField tableField) {
    super(dbConfig, tableInfo, field, tableField);
    this.reflectField = field;
    this.version = field.getAnnotation(Version.class) != null;
    this.property = field.getName();
    this.propertyType = field.getType();
  ...
  public ExtTableFieldInfo(GlobalConfig.DbConfig dbConfig, ExtTableInfo tableInfo,
      Field field) {
    super(dbConfig, tableInfo, field);
    this.reflectField = field;
    this.version = field.getAnnotation(Version.class) != null;
    this.property = field.getName();
    this.propertyType = field.getType();
  ...
  private String convertIf(final String sqlScript, final String property,
    ...
    final FieldStrategy targetStrategy = Optional.ofNullable(fieldStrategy)
        .orElse(this.fieldStrategy);
    ...
    if (targetStrategy == FieldStrategy.NOT_EMPTY && isCharSequence) {
      return SqlScriptUtils
          .convertIf(sqlScript, String.format("%s != null and %s != ''", property, property),
              false);
    }

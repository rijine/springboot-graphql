 ...
 * the License.
 */
package com.yonyou.einvoice.common.metadata.mp.extend;
 ...
 * @since 2018-04-06
 */
public abstract class AbstractExtMethod implements Constants {

  protected static final Log logger = LogFactory.getLog(AbstractExtMethod.class);
  ...
  public void inject(MapperBuilderAssistant builderAssistant, Class<?> mapperClass,
      Class<?> modelClass, ExtTableInfo tableInfo) {
  ...
  protected String sqlComment() {
    return SqlScriptUtils
        .convertChoose(String.format("%s != null and %s != null", WRAPPER, Q_WRAPPER_SQL_COMMENT),
  ...
  protected String sqlSelectColumns(TableInfo table, boolean queryWrapper) {
    ...
    return SqlScriptUtils
        .convertChoose(String.format("%s != null and %s != null", WRAPPER, Q_WRAPPER_SQL_SELECT),
  ...
  protected String sqlWhereByMap(TableInfo table) {
    if (table.isLogicDelete()) {
      ...
      sqlScript = SqlScriptUtils
          .convertIf(sqlScript, String.format("%s != null and !%s", COLUMN_MAP,
   ...
   * @return String
   */
  protected String sqlWhereEntityWrapper(boolean newLine, ExtTableInfo table) {
    if (table.isLogicDelete()) {
   ...
   * @return String
   */
  protected String sqlExtWhereEntityWrapper(boolean newLine, ExtTableInfo table) {
    if (table.isLogicDelete()) {
      String sqlScript = table.getSelfSqlWhere(true, true, WRAPPER_ENTITY_DOT);
      sqlScript = SqlScriptUtils.convertIf(sqlScript, String.format("%s != null", WRAPPER_ENTITY),
          true);
      sqlScript += (NEWLINE + table.getLogicDeleteSql(true, false) + NEWLINE);
      String normalSqlScript = SqlScriptUtils
      ...
      String sqlScript = table.getSelfSqlWhere(false, true, WRAPPER_ENTITY_DOT);
      sqlScript = SqlScriptUtils
          .convertIf(sqlScript, String.format("%s != null", WRAPPER_ENTITY), true);
      sqlScript += NEWLINE;
      sqlScript += SqlScriptUtils.convertIf(String.format(SqlScriptUtils.convertIf(" AND",
   ...
   */
  protected String filterTableFieldInfo(List<TableFieldInfo> fieldList,
      Predicate<ExtTableFieldInfo> predicate,
      Function<ExtTableFieldInfo, String> function, String joiningVal) {
    Stream<TableFieldInfo> infoStream = fieldList.stream();
    if (predicate != null) {
      return infoStream.map(it -> (ExtTableFieldInfo) it).filter(predicate).map(function)
          .collect(joining(joiningVal));
    }
    return infoStream.map(it -> (ExtTableFieldInfo) it).map(function)
        .collect(joining(joiningVal));
  }
  ...
  protected MappedStatement addInsertMappedStatement(Class<?> mapperClass, Class<?> parameterType,
    ...
    return addMappedStatement(mapperClass, id, sqlSource, SqlCommandType.INSERT, parameterType,
        null,
   ...
   * 删除
   */
  protected MappedStatement addDeleteMappedStatement(Class<?> mapperClass, String id,
      SqlSource sqlSource) {
  ...
  protected MappedStatement addUpdateMappedStatement(Class<?> mapperClass, Class<?> parameterType,
      String id,
      SqlSource sqlSource) {
    return addMappedStatement(mapperClass, id, sqlSource, SqlCommandType.UPDATE, parameterType,
        null,
  ...
  protected MappedStatement addMappedStatement(Class<?> mapperClass, String id, SqlSource sqlSource,
    ...
    return builderAssistant
        .addMappedStatement(id, sqlSource, StatementType.PREPARED, sqlCommandType,
